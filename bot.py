...
boss = ["ElJoker63"]  # usuarios supremos
...
Configs = {
    ...
    "gp": None,
    "s": "On",
    "ElJoker63": {"z": 99, "m": "e", "a": "c", "t": "y", "gp": False},
}
...
total_up = {"ElJoker63": {"P": 0, "S": 0}}  # total en gb o megas subidos en bytes (int)
rvs = {"ElJoker63": {"h": "", "u": "", "p": "", "up": "", "z": 0, "m": "m"}}
...
async def start(client: Client, message: Message):
    ...
    if comprobacion_de_user(username) == False:
        await send("⛔ 𝑵𝒐 𝒕𝒊𝒆𝒏𝒆 𝒂𝒄𝒄𝒆𝒔𝒐 @ElJoker63 ⛔")
        return
    else:
        pass
    ...
    msg += f"🗄️**Propietario** : @ElJoker63 🗄️\n"
...
async def download_archive(client: Client, message: Message):
    ...
    for i in downlist[username]:
        ...
        try:
            ...
            if (
                Path(str(root[username]["actual_root"]) + "/" + filename).stat().st_size
                == filesize
            ):
    ...
    try:
        ...
        if Configs[username]["m"] == "u":
            ...
            if len(Urls[username]) >= 10 and username not in boss:
                await msg.edit(
                    "⛔️ 𝑬𝒍 𝒍𝒊𝒎𝒊𝒕𝒆 𝒅𝒆 𝒍𝒊𝒏𝒌𝒔 𝒇𝒖𝒆 𝒑𝒂𝒔𝒂𝒅𝒐 , 𝒖𝒕𝒊𝒍𝒊𝒛𝒆 **/deletelinks**"
                )
                return
            else:
                print("...")
...
async def tg(client: Client, message: Message):
    ...
    try:
        ...
        start = time()
        r = await bot.send_document(
            username,
            path,
            file_name=filename,
            progress=downloadmessage_tg,
            progress_args=(filename, start, msg),
        )
        ...
        if filename.split(".")[-1] == "txt":
            ...
            if "medisur.sld.cu" in urls[0]:
                        ...
                        try:
                            ...
                            async with session.get(du) as r:
                                if r.status == 302 or r.status == 200:
                                    p += 1
                                    await ms.edit(
                                        f"**💢 Borrando enlaces {p}/{t} . Espere por favor 💢**"
                                    )
                        except:
                            pass
                    if p == t:
                        ...
                        await ms.edit(
                            f"**❗️ Solo fueron borrados {p} de {t} archivos**"
                        )
                    os.unlink(path)
                    return
                ...
                async with aiohttp.ClientSession(connector=connector) as session:
                    ...
                    async with session.post(
                        host + "index.php?P=UserLogin", data=payload, ssl=False
                    ) as e:
                        print(222)
                    for url in urls:
                        try:
                            ...
                            async with session.post(
                                f"{host}index.php?P=EditResourceComplete&ID=-{url_id}",
                                data=query,
                                ssl=False,
                            ) as resp:
...
@bot.on_message(
    (filters.regex("https://") | filters.regex("http://")) & filters.private
)
async def down_link(client: Client, message: Message):
    ...
    if comprobacion_de_user(username) == False:
        ...
        pass
    if (
        "youtu.be/" in message.text
        or "twitch.tv/" in message.text
        or "youtube.com/" in message.text
        or "xvideos.com" in message.text
        or "xnxx.com" in message.text
    ):
        list = message.text.split(" ")
        url = list[0]
        try:
        ...
        await client.send_message(
            Channel_Id,
            f"**@{username} Envio un link de #youtube:**\n**Url:** {url}\n**Formato:** {str(format)}p",
        )
        procesos += 1
        download = await ytdlp_downloader(
            url,
            user_id,
            msg,
            username,
            lambda data: download_progres(data, msg, format),
            format,
        )
        if procesos != 0:
            procesos -= 1
        ...
        await client.send_message(
            Channel_Id,
            f"**@{username} Envio un link de #mediafire:**\n**Url:** {url}\n",
        )
        procesos += 1
        file = await download_mediafire(
            url,
            str(root[username]["actual_root"]) + "/",
            msg,
            callback=mediafiredownload,
        )
        if procesos != 0:
            procesos -= 1
        await msg.edit("𝑫𝒆𝒔𝒄𝒂𝒓𝒈𝒂 𝒆𝒙𝒊𝒕𝒐𝒔𝒂")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as r:
                ...
                procesos += 1
                await client.send_message(
                    Channel_Id, f"**@{username} Envio un #link :**\n**Url:** {url}\n"
                )
...
def filezip(fpath: Path, password: str = None, volume=None):
    ...
    if ext_digits < 3:
        ext_digits = 3
    with MultiVolume(
        fpath.with_name(fpath.name + "zip"), mode="wb", volume=volume, ext_digits=0
    ) as archive:
        with SevenZipFile(
            archive, "w", filters=filters, password=password
        ) as archive_writer:
...
async def send_config():
    try:
        await bot.edit_message_text(
            Channel_Id, message_id=db_access, text=dumps(Configs, indent=4)
        )
    except:
...
class MoodleClient:
    def __init__(self, username, password, moodle, proxy):
        ...
        self.session = aiohttp.ClientSession(
            cookie_jar=aiohttp.CookieJar(unsafe=True), connector=proxy
        )
        self.headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Mobile Safari/537.36"
        }
    ...
    async def uploadtoken(self, f, progress, token):
        ...
        async with self.session.post(
            url, data=query, headers=self.headers, ssl=False
        ) as response:
        ...
        async with self.session.post(
            urlw, data=query, headers=self.headers, ssl=False
        ) as response:
            text = await response.text()
        try:
            a = (
                findall(
                    "https?://[^\s\<\>]+[a-zA-z0-9]",
                    loads(text)["event"]["description"],
                )[-1].replace("pluginfile.php/", "webservice/pluginfile.php/")
                + "?token="
                + token
            )
            return a, url
...
class RevistasClient:
    ...
    async def login(self) -> bool:
        async with self.session.get(
            self.host + "login", headers=self.headers, ssl=False
        ) as resp:
    ...
    async def uploadfile_rv(self, file, msg, username):
        ...
        q = Progress(
            filename_god,
            lambda current, total, timestart, filename: uploadfile_progres(
                current, total, timestart, filename, msg
            ),
        )
        data = aiohttp.FormData()
...
def comprobacion_de_user(username):
    if username in Configs or username in boss:
        if exists("downloads/" + str(username) + "/"):
            pass
        else:
        ...
        try:
            ...
            root[username] = {"actual_root": f"downloads/{str(username)}"}
        try:
            downlist[username]
        except:
            downlist[username] = []
...
async def uploaddraft(file, usid, msg, username):
    ...
    if filesize > zipssize:
        ...
        for file in files:
            try:
                upload = await client.LoginUpload(
                    file,
                    lambda size, total, start, filename: uploadfile_progres(
                        size, total, start, filename, msg
                    ),
                )
        ...
        await bot.send_document(
            usid,
            filename + ".txt",
            caption="Gracias por usar nuestros sevicios\nPara continuar subiendo use **/ls** :)",
        )
        if username in boss:
            pass
        else:
    ...
    if filesize - 1048 > zipssize:
        ...
        for path in files:
            while logerrors < 5:
                error_conv = 0
                try:
                    upload = await client.uploadtoken(
                        path,
                        lambda chunk, total, start, filen: uploadfile_progres(
                            chunk, total, start, filen, msg
                        ),
                        token,
                    )
                    ...
                    if mode == "l" or mode == "b":
                        ...
                        upload = upload.replace(
                            "draftfile.php/", "webservice/draftfile.php/"
                        )
                    ...
                    await bot.send_message(
                        usid, f"__**{upload}**__", disable_web_page_preview=True
                    )
                ...
                for file in files:
                    try:
                        ...
                        async with session.get(
                            host + "author/submit", ssl=False
                        ) as resp:
                            print(1)
                        async with session.get(
                            host + "author/submit/1", ssl=False
                        ) as resp:
                            print(2)
                        payload = {
                        ...
                        async with session.post(
                            host + "author/saveSubmit/1", data=payload, ssl=False
                        ) as resp:
                            print(3)
                            print(resp.url)
                        ...
                        async with session.post(
                            post_file_url, data=query, ssl=False
                        ) as resp:
                            text = await resp.text()
                            url = (
                                str(text)
                                .split('"controls"><a href="')[1]
                                .split('">')[0]
                            )
                            links.append(url)
                    except:
...
async def tesisld_api(file, usid, msg, username):
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            payload["F_Password"] = "Michel03."
            async with session.post(
                host + "index.php?P=UserLogin", data=payload, headers=headers, ssl=False
            ) as e:
                print(222)
                print(e.url)
            # upload
            if filesize - 1048 > zipssize:
                ...
                for file in files:
                    try:
                        async with session.get(
                            host + "index.php?P=EditResource&ID=NEW",
                            headers=headers,
                            ssl=False,
                        ) as resp:
                        ...
                        async with session.post(
                            host + f_ids, data=query, headers=headers, ssl=False
                        ) as resp:
                            raw_data = await resp.read()
                            text = raw_data.decode("utf-8", errors="replace")
                        soup = BeautifulSoup(text, "html.parser")
                ...
                async with session.get(
                    host + "index.php?P=EditResource&ID=NEW", headers=h, ssl=False
                ) as resp:
                    print(resp.status)
                    raw_data = await resp.read()
                    text = raw_data.decode("utf-8", errors="replace")
                        ...
                        if filesize - 1048 > zipssize:
                            ...
                            for file in files:
                                try:
                                    ...
                                    post_file_url = (
                                        host + "api/v1/submissions/" + up_id + "/files"
                                    )
                                    fi = Progress(
                                        file,
                                        lambda current, total, timestart, filename: uploadfile_progres(
                        ...
                        if filesize - 1048 > zipssize:
                            ...
                            for file in files:
                                try:
                                    ...
                                    post_file_url = (
                                        host
                                        + "$$$call$$$/wizard/file-upload/file-upload-wizard/upload-file?submissionId="
                                        + up_id
                                        + "&stageId=1&fileStage=2&reviewRoundId=&assocType=&assocId="
                                    )
                                    async with session.post(
                                        post_file_url,
                                        data=query,
                                        ...
                                        if fileId:
                                            url = (
                                                host
                                                + "$$$call$$$/api/file/file-api/download-file?fileId="
                                                + str(fileId)
                                                + "&revision=1&submissionId="
                                                + up_id
                                                + "&stageId=1"
                                            )
                                            links.append(url)
                            ...
                            async with session.post(
                                ...
                                try:
                                    fileId = (
                                        str(text).split('"fileId":')[1].split(",")[0]
                                    )
                                    print(fileId)
                                except:
                                    print("Ahi no se encuentra fileId")
    ...
    if filesize - 1048 > zipssize:
        ...
        for file in files:
            try:
                ...
                query = {"s97304e7e": fi}
                async with session.post(
                    "https://educa.uho.edu.cu/ci_portal_uho/index.php/recursos_pre/my_grocery_recursos_pred/upload_file/archivo",
                    data=query,
                    timeout=60 * 30,
                ) as resp:
        ...
        if len(logslinks) == len(files):
            await msg.delete()
            txtname = (
                "downloads/" + username + "/" + str(filename).split(".")[0] + ".txt"
            )
            with open(txtname, "w") as t:
                message = ""
                lin = ""
...
async def proccess(filex, user_id, msg, username):
    try:
        ...
        if filesize > zipssize:
            ...
            loged = await nextcom(
                user, passw, host, proxy, send, files, msg, username, user_id
            )
            print(21)
            if "Error" not in loged:
                ms = "Links:\n"
